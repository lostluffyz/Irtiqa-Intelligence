...
def _seed_evidence(service: EvidenceService) -> tuple[str, str, str]:
...
def test_evidence_api_list_by_target(client: TestClient) -> None:
    service = EvidenceService()
    score_id, _, _ = _seed_evidence(service)
...
def test_evidence_api_list_by_source(client: TestClient) -> None:
    service = EvidenceService()
    _, _, agent_run_id = _seed_evidence(service)
...
def test_evidence_api_by_company(client: TestClient) -> None:
    service = EvidenceService()
    _, company_id, _ = _seed_evidence(service)
...
def test_evidence_api_by_company_with_type_filter(client: TestClient) -> None:
    service = EvidenceService()
    _, company_id, _ = _seed_evidence(service)
...
def test_evidence_api_summary(client: TestClient) -> None:
    service = EvidenceService()
    score_id, _, _ = _seed_evidence(service)

    response = client.get(
        f"/evidence/summary/{TARGET_TYPE_INTELLIGENCE_SCORE}/{score_id}"
    )
    assert response.status_code == 200
    data = response.json()
    assert data["total_evidence"] >= 2
    assert EVIDENCE_TYPE_COMPUTED_METRIC in data["by_evidence_type"]
    assert RELATIONSHIP_CONTRIBUTES_TO in data["by_relationship_type"]


def test_evidence_api_detail(client: TestClient) -> None:
    service = EvidenceService()
    score_id, _, _ = _seed_evidence(service)
...
def test_evidence_api_list_pagination(client: TestClient) -> None:
    service = EvidenceService()
    score_id, _, _ = _seed_evidence(service)
...
def test_evidence_api_invalid_target_type_in_path(client: TestClient) -> None:
    response = client.get(
        "/evidence/by-target/invalid_type/some-id"
    )
    # The query methods accept any string target_type.
    # Validation against discriminator sets happens at record_evidence time,
    # not at query time. An unrecognized type simply returns empty results.
    assert response.status_code == 200
    data = response.json()
    assert data["items"] == []

...
def test_services_create_and_query_current_entities(service_database) -> None:
    ...
    agent_run = agent_run_service.create(
        company_id=company.id,
    ...
    intent_signal = intent_signal_service.create(
        company_id=company.id,
        contact_id=contact.id,
        website_id=website.id,
    ...
    outreach_message = outreach_service.create(
        company_id=company.id,
        contact_id=contact.id,
    ...
    matched_company = company_service.get_by_domain(company.domain)
    matched_contact = contact_service.get_by_email(contact.email)
    ...
    assert [item.id for item in company_service.search_by_name("Service")] == [company.id]
    assert [item.id for item in company_service.list_by_status("active")] == [company.id]
    assert matched_contact is not None
    assert matched_contact.id == contact.id
    assert [item.id for item in contact_service.list_by_company(company.id)] == [contact.id]
    ...
    assert [item.id for item in intent_signal_service.list_by_type("technology_change")] == [
        intent_signal.id
    ]
    latest_company_score = score_service.latest_for_company(company.id)
    latest_contact_score = score_service.latest_for_contact(contact.id)
    ...
    assert [item.id for item in outreach_service.list_by_status("draft")] == [outreach_message.id]
    assert [item.id for item in agent_run_service.list_by_workflow("test_workflow")] == [
        agent_run.id
    ]


def test_services_raise_structured_errors(service_database) -> None:
    service = CompanyService()
    service.create(name="Conflict Company", domain="conflict.example", status="active")

    with pytest.raises(EntityConflictError) as conflict:
        service.create(name="Duplicate Company", domain="conflict.example", status="active")
